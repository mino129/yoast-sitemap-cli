 ...
 * Author:      The Code Co & Mino
 * Version:     2.0
 *
 * @package yoast-sitemap-cli
 */

namespace YoastCustomCLI;
...
class YoastSitemapCLI {
	...
	const CMD_BUILD_ROOT_SITEMAP = 'yoast-sitemap-build-sitemaps';

	/** The max number of posts per individual sitemap generated by Yoast. */
	const SITEMAP_POSTS_PER_PAGE = 500;
	...
	public function handle_routing( $continue ) {
		...
		if ( preg_match( '{^sitemap_index.xml$}', $request_path ) ) {
			$this->handle_route_sitemap();
		}
		
		// Sub level sitemaps.
		if(preg_match('/([^\/]+?)-sitemap([0-9]+)?\.xml$/', $request_path, $matches)){
			$this->handle_route_sitemap($matches[1], $matches[2]);
		}
	...
	public function cmd_build_root_sitemap() {
		...
		$this->log_info(
			sprintf(
				...
				$this->get_sitemap_cache_file_path()
			)
		);
		...
		$this->log_info('Saving sub level sitemaps to disk');

		$this->build_sitemap_sub_levels();

		$this->log_info('Saved all sub level sitemaps to disk');
	 ...
	 * @return void
	 */
	protected function handle_route_sitemap($post_type = null, $current_page = null) {

		$sitemap_root_cache_file_path = $this->get_sitemap_cache_file_path();
		if ( file_exists( $sitemap_root_cache_file_path ) ) {
			...
			$file_path = ($post_type) 
			? $this->get_sitemap_cache_file_path($post_type . '-sitemap' . $current_page) 
			: $this->get_sitemap_cache_file_path();
			readfile( $file_path );
	...
	protected function build_sitemap_sub_level_cache_file($sitemap_content, $filename) {

		\file_put_contents( // phpcs:ignore
			$this->get_sitemap_cache_file_path($filename),
			$sitemap_content
		);
	...
	/**
	 * Build the sub level sitemaps and dump them to disk.
	 *
	 * @return string
	 */
	protected function build_sitemap_sub_levels() : void {
		$yoast_sitemap = $this->get_yoast_sitemap_instance();
		$yoast_sitemap->init_sitemaps_providers();
		
		// Get all the links from the sitemap providers that Yoast has registered and are needed to be cached.
		$links = [];
		foreach ( $yoast_sitemap->providers as $provider ) {
			$links = array_merge( $links, $provider->get_index_links( self::SITEMAP_POSTS_PER_PAGE ) );
		}

		foreach($links as $link){
			$url = $link['loc'];

			// Extract the post type and page number from the URL. Stole regex from https://github.com/Yoast/wordpress-seo/blob/62ce5d09cf6611f6df63d4248833459d376b9265/inc/sitemaps/class-sitemaps-router.php#L39
			preg_match('/\/([^\/]+?)-sitemap([0-9]+)?\.xml/', $url, $matches);
			$post_type = $matches[1];
			...
			$this->log_info(
				sprintf(
					'Building level sitemap for %s',
					$post_type . '-sitemap' . $current_page
				)
			);

			// Generate the sublevel sitemap in Yoast and dump the output XML.
			ob_start();
				$yoast_sitemap->set_n($current_page ?? 1);
				$yoast_sitemap->build_sitemap($post_type);
				$yoast_sitemap->output();
			$sitemap_content = ob_get_clean();

			// Save the sublevel sitemap to disk with the same filename as Yoast would generate.
			$this->build_sitemap_sub_level_cache_file($sitemap_content, $post_type . '-sitemap' . $current_page);
		}
	 ...
	 * @return string
	 */
	protected function get_sitemap_cache_file_path($filename = 'sitemap_index') {
